...
export class PisoService {
    ...
    listPisos(codigoEdificio: string): Observable<Piso[]> {
        const listPisosEdificio = this.LogisticAPI_URL + "/listPisos?codigoEdificio=" + codigoEdificio;
        const pisos = this.http.get<Piso[]>(listPisosEdificio)!;
        console.log(this.LogisticAPI_URL + "/listPisos?codigoEdificio=" + codigoEdificio);
        return pisos;
    }

    /*
  //Metodo para listar min e max de pisos
  listMinMax(codigoEdificio: string) : Observable<Piso[]>{
      const body.log(body);
      const pisos = this.http.get<Piso[]>(this.LogisticAPI_URL + "/listMinMax", body, { 'headers': headers, observe: 'response' })
      return pisos;
  }
  */
