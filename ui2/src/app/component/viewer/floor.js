...
export default class Floor {
    constructor(parameters) {
        this.onLoad = function (description) {
            ...
            // Store the robot's initial position and direction
            this.initialPosition = this.cellToCartesian(description.initialPosition);
            this.initialDirection = description.initialDirection;

            // Store the floor's elevator location
            this.elevatorLocation = this.cellToCartesian(description.elevatorLocation);

            //Store the floor's hallways
            this.hallwayLocation = [];
            let i = 0;
                while(description.hallwayLocation[i] != null){
                    this.hallwayLocation.push(this.cellToCartesian(description.hallwayLocation[i]));
                    i++;
                }
    ...
    foundElevator(position) {
        return Math.abs(position.x - this.elevatorLocation.x) < 0.5 * this.scale.x && Math.abs(position.z - this.elevatorLocation.z) < 0.5 * this.scale.z
    }

    foundHallway(position){
        let i;
        for(i = 0; i < this.hallwayLocation.length; i++){
            if(Math.abs(position.x - this.hallwayLocation[i].x) < 0.5 * this.scale.x && Math.abs(position.z - this.hallwayLocation[i].z) < 0.5 * this.scale.z)
                return true;
        }
        return false;
    }
