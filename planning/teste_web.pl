...
request_pisos():-
  destroi_pisos(),
  http_open('http://localhost:3000/api/piso/listPisos', ResJSON, [cert_verify_hook(cert_accept_any)]),
...
extrai_pisos([H|[]], [[H.edificio, [H.designacao]]]):-write('Aqui1'),!.
extrai_pisos([H|T], [H2|T2]):-
  write('Aqui'),
  extrai_pisos(T, [[Edi, Des|_]|T3]),
  write('Edificio: '), write(Edi), write('Pisos: '), write(Des).
  %cmp(H.edificio, Edi, T2).

%cmp(Edi, Edi, T2, ):-
%  append(H, )

% A organização vai ficar organizada da seguinte forma: [[A, [A1, A2, A3]], [B, [B1, B2]]]
...
% - 3, corredores;
% - 4, saídas;
request_mapa_pisos():-http_get('http://localhost:3000/api/mapaPiso/listMapaPisos', X, []).
